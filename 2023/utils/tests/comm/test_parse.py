...
from ...comm import Parser  # TODO: not use relative import
...
def test_init_with_both_file_and_text():
    with pytest.raises(ValueError, match="Only one of file_name or text must be provided"):
        Parser(file_name="test.txt", text="Sample text")

def test_get_lines(sample_text):
    parser = Parser(text=sample_text)
    assert parser.get_lines() == ["Line 1", "Line 2", "", "Section 1", "Section 1continued", "", "Line 3", "Line 4"]

def test_get_sections(sample_text):
    parser = Parser(text=sample_text)
    assert parser.get_sections() == ["Line 1\nLine 2", "Section 1\nSection 1continued", "Line 3\nLine 4"]

def test_get_grid(sample_text):
    parser = Parser(text=sample_text)
    grid = parser.get_grid()
    assert grid.content == [["Line", "1"], ["Line", "2"], ["Section", "1"], ["Section", "1continued"], ["Line", "3"], ["Line", "4"]]
